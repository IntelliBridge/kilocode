...
- Modify extension to support an optional `.builder/config.json` and add the addition of `project.id` to the config file there.
...
I don't think it will hurt to save this, particularly since they can remove it by setting `project.id: ""` in `.builder/config.json`.
