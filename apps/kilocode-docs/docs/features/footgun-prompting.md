...
1.  **Override File:** Create a file named `.builder/system-prompt-{mode-slug}` in your workspace root (e.g., `.builder/system-prompt-code` for the Code mode).
...
- **No File, No Override:** If the `.builder/system-prompt-{mode-slug}` file doesn't exist, Kilo Code uses the standard system prompt generation process for that mode.
- **Directory Creation:** Kilo Code ensures the `.builder` directory exists before attempting to read or create the override file.
