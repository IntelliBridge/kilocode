...
- **Project rules**: `.cursor/rules/*.mdc` → `.builder/rules/*.md` (remove YAML frontmatter, keep Markdown content)
- **Legacy rules**: `.cursorrules` → `.builder/rules/legacy-rules.md`
- **AGENTS.md**: Works identically in Kilo Code (no conversion needed)
- **Global rules**: Recreate in `~/.builder/rules/*.md` directory

Kilo Code also supports mode-specific rules (`.builder/rules-{mode}/`), which Cursor and Windsurf don't have. This allows different rules for different workflows (e.g., Code mode vs Debug mode).
