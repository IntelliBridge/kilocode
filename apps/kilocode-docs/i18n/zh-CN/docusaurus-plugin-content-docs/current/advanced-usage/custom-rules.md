...
自定义规则主要从 **`.builder/rules/` 目录** 加载。这是组织项目特定规则的推荐方法。每个规则通常放置在具有描述性名称的 Markdown 文件中：
...
Kilo Code 提供了一个内置界面，用于管理您的自定义规则，而无需手动编辑 `.builder/rules/` 目录中的文件。要访问 UI，请单击 Kilo Code 窗口**右下角**的 <Codicon name="law" /> 图标。
...
1.  存放在 `~/.builder/rules/` 目录的**全局规则**
2.  存放在 `.builder/rules/` 目录的**项目规则**
3.  **旧版回退文件**（为了向后兼容）：
    - `.roorules`
    - `.clinerules`
    - `.kilocoderules`（已弃用）
...
:::note
我们强烈建议将规则保留在 `.builder/rules/` 文件夹中，因为它提供了更好的组织，并且是未来版本的首选方法。基于文件夹的结构允许更细粒度的规则组织和更清晰的关注点分离。基于文件的旧方法为了向后兼容而保留，但可能会在未来的版本中发生变化。
:::
...
1.  如果 `~/.builder/rules/` 目录尚不存在，则创建它
...
:::tip 专业提示：基于文件的团队标准
在团队环境中工作时，将 `.builder/rules/codestyle.md` 文件置于版本控制之下，可以让你在整个开发团队中标准化 Kilo 的行为。这确保了项目中的每个人都有一致的代码风格、文档实践和开发工作流程。
:::
