...
export class CLI {
	...
	async initialize(): Promise<void> {
		...
		try {
			logs.info("Initializing Codeforce CLI...", "CLI")
			...
			if (supportsTitleSetting()) {
				process.stdout.write(`\x1b]0;Codeforce - ${folderName}\x07`)
			}
			...
			logs.info("Codeforce CLI initialized successfully", "CLI")
		} catch (error) {
	...
	async dispose(signal?: string): Promise<void> {
		...
		try {
			logs.info("Disposing Codeforce CLI...", "CLI")
			...
			logs.info("Codeforce CLI disposed", "CLI")
		} catch (error) {
