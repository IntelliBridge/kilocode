...
describe("loadRuleFiles", () => {
	...
	it("should not combine content from multiple rule files when they exist", async () => {
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			if (filePath.toString().endsWith(".builderrules")) {
	...
	it("should use .builder/rules/ directory when it exists and has files", async () => {
		// Simulate .builder/rules directory exists
		...
		readdirMock.mockResolvedValueOnce([
			{
				...
				parentPath: "/fake/path/.builder/rules",
			},
			{
				name: "file2.txt",
				isFile: () => true,
		...
		statMock.mockImplementation((path) => {
			...
			if (
				normalizedPath.includes("/fake/path/.builder/rules/file1.txt") ||
				normalizedPath.includes("/fake/path/.builder/rules/file2.txt")
			) {
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			if (normalizedPath === "/fake/path/.builder/rules/file1.txt") {
				return Promise.resolve("content of file1")
			}
			if (normalizedPath === "/fake/path/.builder/rules/file2.txt") {
		...
		const expectedRelativePath1 =
			process.platform === "win32" ? ".builder\\rules\\file1.txt" : ".builder/rules/file1.txt"
		const expectedRelativePath2 =
			process.platform === "win32" ? ".builder\\rules\\file2.txt" : ".builder/rules/file2.txt"
		...
		const expectedRulesDir =
			process.platform === "win32" ? "\\fake\\path\\.builder\\rules" : "/fake/path/.builder/rules"
		const expectedFile1Path =
			process.platform === "win32"
				? "\\fake\\path\\.builder\\rules\\file1.txt"
				: "/fake/path/.builder/rules/file1.txt"
		const expectedFile2Path =
			process.platform === "win32"
				? "\\fake\\path\\.builder\\rules\\file2.txt"
				: "/fake/path/.builder/rules/file2.txt"
	...
	it("should fall back to .builderrules when .builder/rules/ is empty", async () => {
		// Simulate .builder/rules directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: vi.fn().mockReturnValue(true),
		} as any)
	...
	it("should read files from nested subdirectories in .builder/rules/", async () => {
		// Simulate .builder/rules directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: vi.fn().mockReturnValue(true),
		} as any)
		...
		readdirMock.mockResolvedValueOnce([
			...
			},
			{
				...
				isSymbolicLink: () => false,
				isDirectory: () => false,
				parentPath: "/fake/path/.builder/rules/subdir",
			},
			{
				name: "nested2.txt",
				isFile: () => true,
				isSymbolicLink: () => false,
				isDirectory: () => false,
				parentPath: "/fake/path/.builder/rules/subdir/subdir2",
			},
		] as any)
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			// Handle both Unix and Windows path separators
			const normalizedPath = pathStr.replace(/\\/g, "/")
			if (normalizedPath === "/fake/path/.builder/rules/root.txt") {
				return Promise.resolve("root file content")
			}
			if (normalizedPath === "/fake/path/.builder/rules/subdir/nested1.txt") {
				return Promise.resolve("nested file 1 content")
			}
			if (normalizedPath === "/fake/path/.builder/rules/subdir/subdir2/nested2.txt") {
		...
		const expectedRelativeRootPath =
			process.platform === "win32" ? ".builder\\rules\\root.txt" : ".builder/rules/root.txt"
		const expectedRelativeNested1Path =
			process.platform === "win32" ? ".builder\\rules\\subdir\\nested1.txt" : ".builder/rules/subdir/nested1.txt"
		const expectedRelativeNested2Path =
			process.platform === "win32"
				? ".builder\\rules\\subdir\\subdir2\\nested2.txt"
				: ".builder/rules/subdir/subdir2/nested2.txt"
		...
		const expectedRootPath2 =
			process.platform === "win32"
				? "\\fake\\path\\.builder\\rules\\root.txt"
				: "/fake/path/.builder/rules/root.txt"
		const expectedNested1Path2 =
			process.platform === "win32"
				? "\\fake\\path\\.builder\\rules\\subdir\\nested1.txt"
				: "/fake/path/.builder/rules/subdir/nested1.txt"
		const expectedNested2Path2 =
			process.platform === "win32"
				? "\\fake\\path\\.builder\\rules\\subdir\\subdir2\\nested2.txt"
				: "/fake/path/.builder/rules/subdir/subdir2/nested2.txt"
...
describe("addCustomInstructions", () => {
	...
	it("should combine all instruction types when provided", async () => {
		// Simulate no .builder/rules-test-mode directory
		statMock.mockRejectedValueOnce({ code: "ENOENT" })
		...
		expect(result).toContain("Rules from .builderrules-test-mode:\nmode specific rules")
	})
	...
	it("should use .builder/rules-test-mode/ directory when it exists and has files", async () => {
		// Simulate .builder/rules-test-mode directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: vi.fn().mockReturnValue(true),
		} as any)
		...
		readdirMock.mockResolvedValueOnce([
			{
				...
				isFile: () => true,
				isSymbolicLink: () => false,
				parentPath: "/fake/path/.builder/rules-test-mode",
			},
			{
				name: "rule2.txt",
				isFile: () => true,
		...
		statMock.mockImplementation((path) => {
			...
			const normalizedPath = path.toString().replace(/\\/g, "/")
			if (
				normalizedPath.includes("/fake/path/.builder/rules-test-mode/rule1.txt") ||
				normalizedPath.includes("/fake/path/.builder/rules-test-mode/rule2.txt")
			) {
				return Promise.resolve({
					isFile: vi.fn().mockReturnValue(true),
				}) as any
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			// Handle both Unix and Windows path separators
			const normalizedPath = pathStr.replace(/\\/g, "/")
			if (normalizedPath === "/fake/path/.builder/rules-test-mode/rule1.txt") {
				return Promise.resolve("mode specific rule 1")
			}
			if (normalizedPath === "/fake/path/.builder/rules-test-mode/rule2.txt") {
		...
		const expectedRelativeRule1Path =
			process.platform === "win32" ? ".builder\\rules-test-mode\\rule1.txt" : ".builder/rules-test-mode/rule1.txt"
		const expectedRelativeRule2Path =
			process.platform === "win32" ? ".builder\\rules-test-mode\\rule2.txt" : ".builder/rules-test-mode/rule2.txt"
		...
		const expectedAbsTestModeDir =
			process.platform === "win32"
				? "\\fake\\path\\.builder\\rules-test-mode"
				: "/fake/path/.builder/rules-test-mode"
		const expectedAbsRule1Path =
			process.platform === "win32"
				? "\\fake\\path\\.builder\\rules-test-mode\\rule1.txt"
				: "/fake/path/.builder/rules-test-mode/rule1.txt"
		const expectedAbsRule2Path =
			process.platform === "win32"
				? "\\fake\\path\\.builder\\rules-test-mode\\rule2.txt"
				: "/fake/path/.builder/rules-test-mode/rule2.txt"
	...
	it("should fall back to .builderrules-test-mode when .builder/rules-test-mode/ does not exist", async () => {
		// Simulate .builder/rules-test-mode directory does not exist
		statMock.mockRejectedValueOnce({ code: "ENOENT" })

		// Simulate .builderrules-test-mode exists
		readFileMock.mockImplementation((filePath: PathLike) => {
			if (filePath.toString().includes(".builderrules-test-mode")) {
	...
	it("should correctly format content from directories when using .builder/rules-test-mode/", async () => {
		...
		readdirMock.mockResolvedValueOnce([
			{ name: "rule1.txt", isFile: () => true, parentPath: "/fake/path/.builder/rules-test-mode" },
		] as any)
		readFileMock.mockReset()
...
describe("Rules directory reading", () => {
	it.skipIf(process.platform === "win32")("should follow symbolic links in the rules directory", async () => {
			...
			.mockResolvedValueOnce([
				{
					...
					isFile: () => true,
					parentPath: "/fake/path/.builder/rules/symlink-target-dir",
				},
			] as any)
		...
		statMock.mockImplementation((path: string) => {
			// For directory check
			if (path === "/fake/path/.builder/rules" || path.endsWith("dir")) {
				return Promise.resolve({
					isDirectory: vi.fn().mockReturnValue(true),
					isFile: vi.fn().mockReturnValue(false),
				} as any)
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			// Handle both Unix and Windows path separators
			const normalizedPath = pathStr.replace(/\\/g, "/")
			if (normalizedPath === "/fake/path/.builder/rules/regular.txt") {
				return Promise.resolve("regular file content")
			}
			if (normalizedPath === "/fake/path/.builder/symlink-target.txt") {
				return Promise.resolve("symlink target content")
			}
			if (normalizedPath === "/fake/path/.builder/rules/symlink-target-dir/subdir_link.txt") {
				return Promise.resolve("regular file content under symlink target dir")
			}
			if (normalizedPath === "/fake/path/.builder/nested-symlink-target.txt") {
		...
		const expectedRelativeRegularPath =
			process.platform === "win32" ? ".builder\\rules\\regular.txt" : ".builder/rules/regular.txt"
		const expectedRelativeSymlinkPath =
			process.platform === "win32" ? ".builder\\symlink-target.txt" : ".builder/symlink-target.txt"
		const expectedRelativeSubdirPath =
			process.platform === "win32"
				? ".builder\\rules\\symlink-target-dir\\subdir_link.txt"
				: ".builder/rules/symlink-target-dir/subdir_link.txt"
		const expectedRelativeNestedPath =
			process.platform === "win32" ? ".builder\\nested-symlink-target.txt" : ".builder/nested-symlink-target.txt"
		...
		expect(readlinkMock).toHaveBeenCalledWith("/fake/path/.builder/rules/link.txt")
		expect(readlinkMock).toHaveBeenCalledWith("/fake/path/.builder/rules/link_dir")

		// Verify both files were read
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.builder/rules/regular.txt", "utf-8")
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.builder/symlink-target.txt", "utf-8")
		expect(readFileMock).toHaveBeenCalledWith(
			"/fake/path/.builder/rules/symlink-target-dir/subdir_link.txt",
			"utf-8",
		)
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.builder/nested-symlink-target.txt", "utf-8")
	})
	beforeEach(() => {
		vi.clearAllMocks()
	})
	...
	it.skipIf(process.platform === "win32")("should correctly format multiple files from directory", async () => {
		...
		// Simulate listing files
		readdirMock.mockResolvedValueOnce([
			{ name: "file1.txt", isFile: () => true, parentPath: "/fake/path/.builder/rules" },
			{ name: "file2.txt", isFile: () => true, parentPath: "/fake/path/.builder/rules" },
			{ name: "file3.txt", isFile: () => true, parentPath: "/fake/path/.builder/rules" },
		] as any)
		...
		statMock.mockImplementation((path) => {
			...
			expect([
				"/fake/path/.builder/rules/file1.txt",
				"/fake/path/.builder/rules/file2.txt",
				"/fake/path/.builder/rules/file3.txt",
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			if (normalizedPath === "/fake/path/.builder/rules/file3.txt") {
		...
		const expectedRelativeFile3Path =
			process.platform === "win32" ? ".builder\\rules\\file3.txt" : ".builder/rules/file3.txt"
