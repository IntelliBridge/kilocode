...
export function getSystemPromptFilePath(cwd: string, mode: Mode): string {
	// kilocode_change
	return path.join(cwd, ".builder", `system-prompt-${mode}`)
}

/**
 * Loads custom system prompt from a file at .builder/system-prompt-[mode slug]
...
/**
 * Ensures the .builder directory exists, creating it if necessary
 */
export async function ensureRooDirectory(cwd: string): Promise<void> {
	// kilocode_change
	const rooDir = path.join(cwd, ".builder")
